...
- Participant #2: needs a Git-free entry point via the web
- Egor: wants to try it "with one button," then learn the details
- abcdef0101: VM image as a third delivery method

**Analogy:** WordPress.org (self-hosted) vs WordPress.com (hosted). Both deliver the same product via different entry points. A WordPress.com user can export everything and switch to self-hosted.

## Problem

1. **Entry barrier.** Git + CLI + fork + setup.sh ≈ 30–60 minutes for a newcomer, with a risk of errors.
2. **No managed onboarding.** setup.sh asks 7 questions at once — no gradual entry path.
...
User manages files directly                Agent manages files on behalf of the user
Updates: bash update.sh                    Updates: agent proposes automatically
```

Both layers work with the same distribution (FMT-exocortex-template). Platform hosting is a **thin layer** on top — not a separate product.
...
│  • Peer partner agent: regular work via chat            │
...
The user moves through levels at their own pace (not by week):
...
The hosting layer manages IWE via the same scripts, without interactive prompts:

```bash
# Initial T1-level installation (no prompts)
bash setup.sh \
  --level=T1 \
  --yes \
  GITHUB_USER=alice \
...
# → user is in a tmux session inside ~/IWE/
```

The Vagrant image is a third delivery method (for a DevOps audience and for testing).
...
## Rejected decision: "v2 from scratch"

**Rejected (Mar 27, 2026).** IWE is a distribution (rolling release, not major versions). WP-180 implements SOTA incrementally. A doubled budget is unrealistic at 60h/week. Incremental stabilization was chosen.

## ArchGate (EMOGSSB)

| Characteristic | Score | Rationale |
|----------------|-------|-----------|
| Evolvability | 9 | One distribution, two delivery layers. The platform does not fork the code |
| Scalability | 8 | Git hosting scales using standard methods |
...
| Security | 8 | Private repositories, git bundle export |

**Result:** 60/70 — PASS (≥8 on all characteristics including Security ≥8).

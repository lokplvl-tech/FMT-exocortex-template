...
## L1 — Platform (do not modify without good reason)
...
**What happens if you edit L1:** the next `update.sh` run will overwrite your changes. To modify behavior, do it through L3.
...
**Example:** you want Day Open to check tasks in Linear — add the following to `extensions/day-open.before.md`:
```
...
governance_repo: DS-strategy   # name of your strategy repository
...
**When to use it:** you have defined a new behavior rule for Claude, tested it in practice, and want to make it part of the Platform (get it into L1 and into the template for all users).

**Cycle:**
1. New behavior → `CLAUDE.md §9` (authoring) + a line in `STAGING.md` (status: testing)
2. Stable for several weeks → `status: validated`
...
| My rule is only for me | `CLAUDE.md §9` (L3) |

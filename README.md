...
Just as an IDE unifies an editor, compiler, and debugger into a single environment for a programmer — IWE unifies Knowledge, planning, and AI agents into a single environment for thinking.
...
> **Core Principle: exoskeleton, not prosthetic.** IWE amplifies your thinking — it does not replace it. After every Session you become more competent, not just better informed. Learn more: [principles-vs-skills.md](docs/principles-vs-skills.md).
...
IWE is not a set of prompts. It is a **work culture**: 14 elements (Protocols, Skills, formats) that transform chaotic AI interaction into a managed Process. Learn more: [DP.M.008 — IWE Operating Rules](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/03-methods/DP.M.008-iwe-operating-rules.md).
...
What you learn does not stay only in your head. Valuable Knowledge is formalized into a Pack — a Domain passport. Pack is the single source of truth for domain Knowledge. Learn more: [LEARNING-PATH.md](docs/LEARNING-PATH.md).
...
- **Everyone doing intellectual work** — and wanting a **symbiosis with AI**, not dependence on it. An exoskeleton for thinking, not a prosthetic.
...
|----------|---------------|-----------|
| **Product development** | Claude knows the Architecture, tech debt, and Roadmap. Every Session is a continuation, not a restart from scratch. | [SC.013](docs/use-cases/USE-CASES.md#sc013-рабочая-сессия-с-claude-code), [SC.015](docs/use-cases/USE-CASES.md#sc015-развитие-системы-ds) |
| **Documentation management** | Knowledge is captured into Pack as work progresses. No need to "write the docs later" — they are written during the work. | [SC.004](docs/use-cases/USE-CASES.md#sc004-фиксация-и-экстракция-знаний), [SC.014](docs/use-cases/USE-CASES.md#sc014-формализация-знаний-pack) |
...
| **Writing articles** | A creative Pipeline: note → draft → ready piece → publication. Every Artifact is tracked. | [SC.005](docs/use-cases/USE-CASES.md#sc005-публикация-контента) |
...
Learn more: [extensions/README.md](extensions/README.md)
...
A: For a full setup (Claude Code) — Claude Pro ($20/month) is recommended. If needed, you can upgrade to Claude Max (~$100/month) for unlimited usage. For a minimal setup (`setup.sh --core`) — works with any AI CLI. Learn more: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md).

**Q: Does it work with other AI models (not Claude)?**
...
Other agents (Cursor, Copilot, Gemini) require adaptation. Learn more: [PORTABILITY.md](docs/PORTABILITY.md).
The minimal setup (`setup.sh --core`) works without being tied to a specific agent.

**Q: Does it work on Linux/Windows?**
A: Yes. The core works on any OS. Strategist automation: macOS — launchd, Linux — cron, Windows — WSL. Learn more: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md).
...
**Q: What is a Pack?**
A: A formalized Knowledge Domain — the single source of truth for domain Knowledge. Learn more: [LEARNING-PATH.md](docs/LEARNING-PATH.md).

**Q: Is my data secure?**
A: Three protection zones: local, GitHub (private repositories), Platform (per-user isolation). Learn more: [DATA-POLICY.md](docs/DATA-POLICY.md).
...
A: Yes. Claude Code + CLAUDE.md + memory/ work fully on their own. The Strategist provides planning automation. Without it, you plan manually.

**Q: How do I configure the strategizing day?**
A: In `memory/day-rhythm-config.yaml`, change `strategy_day: sunday` to the desired day. Learn more: [LEARNING-PATH.md](docs/LEARNING-PATH.md).

**Q: The clone landed in `~` instead of `~/IWE`?**
A: All setup commands must be run in the same terminal. If you opened a new one, it starts from `~`. Delete the folder from `~` and repeat starting with `cd ~/IWE`. Learn more: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md).
...
**The IWE community** is a closed chat for Practitioners working within the same system: ORZ, Pack, Exocortex. A place to discuss not "how to write better prompts," but how to build serious intellectual work.
